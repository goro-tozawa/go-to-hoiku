class ChildcareUser < ApplicationRecord
  ...
  devise :database_authenticatable, 
   ...
   with_options presence: true do
      ...
      validates :facility_address,format: {with: /\A[ぁ-んァ-ン一-龥々]+\z/, message: "The facility address is invalid. Enter double-byte characters."}
      validates :facility_phone_number, format: { with:/\A\d{11}\z/, message: "Please write within 11 digits with only numbers" }
      validates :representative,format: { with: /\A[ぁ-んァ-ン一-龥々]+\z/, message: "The representative nameis invalid. Input full-width characters."}
